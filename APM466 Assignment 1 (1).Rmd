...
By using formula $P(B)=\sum_i^np_i(1+r/2)^{-t(B)2}$, we can get the YTM for all 11 bonds in 10 days, where P(B) is the dirty price and r(B) is the YTM. Then, plotting the yield curve we get the Figure 1, where each line represent the yield curve of each day. \
...
By using formula $P=\sum_i^np_i(1+r/2)^{-t_i2}$ and bootstrapping method $P=\sum_i^{n-1}(1+r/2)^{-t_i2}+p_n(1+r/2)^{-t_n2}$, where the bond compounds semi-annually, $t_i$ for $i<n$ is known and $t_n$ is unknown, we compute the spot rate at each $t_n$. Recursively, we can get the spot curve in Figure 3.
...
Similarly to part 4(a), we can use linear interpolation and get the spot curve with spot rate at time {1,2,3,4,5} years. Then applying the value to $r_{12}=-\frac{logP(t,T_2-logP(t,T_2))}{T_2-T_1}$, we can get the forward curve in Figure 4.
...
https://github.com/Lynn-Y-L/APM466
